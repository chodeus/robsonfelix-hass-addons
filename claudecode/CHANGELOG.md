...
### Fixed
- **`claude update` no longer reports a bogus SSE4.2 CPU error after a successful update.** The 2.8.0 smoke test ran `timeout 30 command claude --version`, but `command` is a shell builtin and `timeout` is an external binary — so `timeout` tried to exec a program named `command`, got ENOENT, and exited 127. `2>/dev/null` hid the real message (`timeout: failed to run command 'command'`), the version came back empty, and the wrapper fell through to its "may need a CPU with SSE4.2" branch. It fired on **every** update since 2.8.0 no matter what npm did: the install itself was fine, only the verdict was wrong. `timeout` resolves `claude` from PATH and can't re-enter the shell function, so `command` was never needed. Only the `claude update` wrapper was affected — the startup and hourly auto-update checks in `run.sh` were always written without `command` and worked correctly
- **A failed `claude update` now reports the actual error instead of guessing.** The smoke test's stderr is captured and printed rather than discarded; SSE4.2 is only named when the host actually implicates it (no `sse4_2` in `/proc/cpuinfo`, or exit 132/SIGILL or 124/timeout); and a surviving placeholder stub is detected and reported as "postinstall did not complete" — the real failure mode now that Claude Code ships its native binary as a separate platform package. The wrapper also returns a non-zero exit status on failure instead of always returning 0
- **Every `npm install` now runs with `--foreground-scripts`** — the `claude update` wrapper plus the startup and hourly auto-update paths (install and rollback). Claude Code now ships its native ~256MB binary as a separate platform package that the postinstall (`install.cjs`) puts in place; npm hides lifecycle output by default, so a failed placement would still print "changed 2 packages" and read as a clean install, in the terminal and in the add-on log alike
- **`claude update` pre-flights free space** on `/homeassistant` (~600MB needed) rather than half-installing into a full config partition
- **`gh` credentials now survive add-on updates.** `gh` keeps its token in `$GH_CONFIG_DIR`, defaulting to `/root/.config/gh` — the container layer, which is rebuilt from the image on every update/rebuild. Claude's own state persists (`/root/.claude` → `.claudecode`) but nothing covered `gh`, so every add-on update silently threw the login away and the next `gh`/`git push` demanded a fresh `gh auth login`. It now points at `/data/gh`, which survives updates and rebuilds and is cleared only on uninstall. Deliberately **not** under `/homeassistant`: that tree is captured by HA backups, and the token would ride along in every archive. gh's git credential helper is re-established at boot too — it lives in `/root/.gitconfig`, equally ephemeral, so persisting the token alone would have left `git push` prompting for a username while the login was still perfectly valid

### Changed
- **AppArmor: added `l` (link) to `/homeassistant/**`.** Claude Code's postinstall hardlinks its ~256MB native binary out of the platform package; the profile granted `rwk` but not `l`, so `linkSync` failed with EACCES and `install.cjs` fell back to a full copy — leaving two 256MB files (490MB) in `.claudecode/npm-global`, all of it swept into every HA backup. With link permission the install matches the image's own layout (one inode, two names, 245MB). Purely a footprint fix; hardlinks can't cross filesystems and the profile already granted full read/write there
